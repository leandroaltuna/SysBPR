...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_column != 'username1')
		{
...
// WHERE roles.active = 1 AND groups_roles.active = 1 AND users_groups.user_id = 1
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.back, '.$this->tables['groups'].'.icon')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
